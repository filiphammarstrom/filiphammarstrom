...
export async function GET(req: Request) {
  ...
  try {
    ...
    const filename = `momsdeklaration-${from.getFullYear()}${String(from.getMonth() + 1).padStart(2, "0")}.xml`;
    ...
    return NextResponse.json({ error: "Kunde inte exportera moms-XML" }, { status: 500 });
  }
}
